    ...
    ep_store.update(&mut cx, |store, _cx| {
        ...
        if let PredictionProvider::Zeta2(format) = provider {
            if format != ZetaFormat::default() {
                let model_id = std::env::var("ZERMINAL_ZETA_MODEL").ok();
                let environment = std::env::var("ZERMINAL_ZETA_ENVIRONMENT").ok();
...
pub async fn predict_baseten(
    ...
    let model_id =
        std::env::var("ZERMINAL_ZETA_MODEL").context("ZERMINAL_ZETA_MODEL environment variable required")?;
