...
//! regression. Set `Z3RM_TEST_SANDBOX_REQUIRE_ENFORCED=1` to turn that skip into
...
mod imp {
    ...
    fn run() -> Result<()> {
        let require_enforced = env_flag("Z3RM_TEST_SANDBOX_REQUIRE_ENFORCED");
        let wsl = Wsl::detect();
    ...
    fn not_enforced(require_enforced: bool, reason: &str) -> Result<()> {
        if require_enforced {
            bail!(
                "Z3RM_TEST_SANDBOX_REQUIRE_ENFORCED is set, but the WSL sandbox could not be \
                 enforced: {reason}"
            );
        }
        println!(
    ...
    fn check_env_forwarding(checks: &mut Checks) -> Result<()> {
        ...
        env.insert("Z3RM_TEST_FORWARDED".to_string(), "yes".to_string());
        ...
        let outcome = drive_sandbox(
            ...
            &[
                "-c",
                "[ \"$Z3RM_TEST_FORWARDED\" = yes ] && [ \"$PATH\" != /zed-sentinel-should-not-win ]",
            ],
            &[],
            SandboxPermissions::default(),
            &env,
    ...
    fn discover_peer(wsl: &Wsl) -> Result<Option<String>> {
        if let Some(address) = std::env::var("Z3RM_TEST_ECHO_ADDR")
            .ok()
    ...
    fn ensure_host_port(address: &str) -> Result<()> {
        let (host, port) = address
            .rsplit_once(':')
            .with_context(|| format!("Z3RM_TEST_ECHO_ADDR must be host:port, got {address:?}"))?;
        ensure!(
            !host.is_empty() && port.parse::<u16>().is_ok(),
            "Z3RM_TEST_ECHO_ADDR must be host:port, got {address:?}"
        );
        Ok(())
    }
