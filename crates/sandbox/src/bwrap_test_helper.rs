...
mod imp {
    ...
    fn run_checks() -> Result<()> {
        ...
        unsafe {
            std::env::set_var("Z3RM_SANDBOX_PROXY_ALLOW_LOCAL_IPS", "1");
        }

        let checks_path =
            std::env::var("Z3RM_SANDBOX_CHECKS").context("Z3RM_SANDBOX_CHECKS must be set")?;
        ...
        let echo_port =
            std::env::var("Z3RM_TEST_ECHO_PORT").unwrap_or_else(|_| DEFAULT_ECHO_PORT.to_string());
