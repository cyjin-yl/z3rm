...
Use the `ZERMINAL_LOG` environment variable to control logging output for Zerminal
...
```
ZERMINAL_LOG=info,project=debug,agent=off
```
...
- You don't need to specify the global level, default is `trace` in the crate
  and `info` set by `RUST_LOG` in Zerminal.
