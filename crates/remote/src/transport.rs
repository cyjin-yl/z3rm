...
async fn build_remote_server_from_source(
    ...
    if let Ok(path) = std::env::var("ZERMINAL_COPY_REMOTE_SERVER") {
        let path = std::path::PathBuf::from(path);
        if path.exists() {
            return Ok(Some(path));
        } else {
            log::warn!(
                "ZERMINAL_COPY_REMOTE_SERVER path does not exist, falling back to ZERMINAL_BUILD_REMOTE_SERVER: {}",
                path.display()
            );
        }
    }
    ...
    let build_remote_server =
        std::env::var("ZERMINAL_BUILD_REMOTE_SERVER").unwrap_or("nocompress".into());
    ...
    if let "never" = &*build_remote_server {
        ...
        log::warn!("ZERMINAL_BUILD_REMOTE_SERVER is disabled, but no server binary exists on the server")
    }
    ...
    if platform.os == RemoteOs::Linux && use_musl {
        ...
        if let Ok(path) = std::env::var("ZERMINAL_ZSTD_MUSL_LIB") {
