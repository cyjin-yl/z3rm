...
You can confirm that Zeta is properly configured by opening the [Settings Editor](z3rm://settings/edit_predictions.providers) (`Cmd+,` on macOS or `Ctrl+,` on Linux/Windows) and searching for `edit_predictions`. The `provider` field should be set to `Zed AI`.
