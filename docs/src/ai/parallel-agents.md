...
To automate setup steps whenever a new worktree is created, use a [Task hook](../tasks.md#hooks). The `create_worktree` hook runs automatically after Zed creates a linked worktree, with `ZERMINAL_WORKTREE_ROOT` pointing at the new worktree and `ZERMINAL_MAIN_GIT_WORKTREE` pointing at the original repository.
