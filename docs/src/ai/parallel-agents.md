...
To automate setup steps whenever a new worktree is created, use a [Task hook](../tasks.md#hooks). The `create_worktree` hook runs automatically after Zed creates a linked worktree, with `Z3RM_WORKTREE_ROOT` pointing at the new worktree and `Z3RM_MAIN_GIT_WORKTREE` pointing at the original repository.
