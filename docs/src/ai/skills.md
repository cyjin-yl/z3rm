...
Open the Settings Editor (`Cmd+,` on macOS, `Ctrl+,` on Linux/Windows) and navigate to **AI > Skills**, or go directly to [agent.skills](z3rm://settings/agent.skills).

The **User** tab shows your global skills, and each **Project** tab shows the skills for that project.

For each skill you can:

- **Copy Share Link** — copies a `z3rm://skill` link that embeds the skill, ready to send to someone else (see [Sharing Skills](#sharing-skills))
...
You can hand a skill to a teammate without hosting it anywhere. In the Skills settings page, click the **link** icon on a skill row to copy a `z3rm://skill?data=…` link to your clipboard.
